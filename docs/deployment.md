...
> **✅ Provisionné par Terraform — voir `infra/` (`infra/README.md`).** Le provisioning réel se fait
> désormais en **IaC déclaratif** (`terraform init/plan/apply`), pas par la séquence `az` impérative
> ci-dessous (conservée comme référence/explication). L'`infra/main.tf` crée RG + Azure SQL S0 + ACR +
> Container Apps avec une **identité managée user-assigned** (AcrPull).
>
> **⚠️ Particularités de l'abonnement « Azure for Students » (rencontrées au déploiement 2026-06-16) :**
> - **ACR Tasks est DÉSACTIVÉ** (`TasksOperationsNotAllowed`) → `az acr build` impossible. L'image se
>   construit avec le **SDK .NET sans Docker** : `dotnet publish /t:PublishContainer` (cf.
>   **`infra/deploy-backend.ps1`** pour un redéploiement reproductible). Le workflow
>   `azure-container-app.yml` a été réécrit en ce sens (gated `AZURE_CREDENTIALS`).
> - **`registry { identity = "System" }`** fait expirer la révision → utiliser une identité **user-assigned**
>   créée avant l'app (AcrPull pré-attribué). Image d'amorçage doit écouter sur **8080**.
> - **CD automatique non câblé** faute de service principal (le tenant Cégep peut bloquer
>   `az ad sp create-for-rbac`). Redéploiement backend = `infra/deploy-backend.ps1` (manuel). Si un SP
>   devient disponible : `gh secret set AZURE_CREDENTIALS` active le workflow.
> - **Frontend SWA** laissé en *Central US* : un Static Web App sert depuis l'**edge mondial** (la région
>   n'affecte pas la latence) et le Canada n'est pas une région SWA.
