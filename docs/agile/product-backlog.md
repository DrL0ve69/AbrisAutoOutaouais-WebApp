...
| US-7.1 | En tant que **client**, je veux **payer ma commande/location/installation par Interac e-Transfer** (demande par courriel/téléphone/ID) afin de payer sans carte de crédit. | 13 | **Could** | ✅ **LIVRÉ & MERGÉ (2026-06-23, PR #59)** — épic e-Transfer complet (Commandes 7.1 + Locations 7.2 + Installations 7.3), revue de fin d'épic indépendante APPROVE WITH NITS ; **location = montant TOTAL du contrat** (décision propriétaire) ; statut porté par les agrégats (`PaymentInfo`), réf base32 non devinable, admin « marquer payé », i18n fr/en, e2e dual-thème. |
...
| US-7.3 | En tant qu'**équipe**, je veux un **port `IPaymentService` + adaptateurs** (manuel/VoPay/Paysafe) sélectionnés par config et des **webhooks signés idempotents** afin d'encaisser de façon asynchrone et fiable. | 13 | **Could** | ✅ **LIVRÉ & MERGÉ (2026-06-23, PR #59)** — port `IPaymentService` + adaptateurs sélectionnés par config ; **stubs keyless VoPay/Paysafe** (garde DI fail-fast, jamais activés — règle budget) ; **patron webhook signé/idempotent documenté non codé** (spike §4.5 + `Services/Payments/README.md`). Adaptateurs payants non activés. |
...
| US-15.2 | En tant que **client**, je veux **un seul champ d'adresse** (n° civique + rue) avec province/pays/**code postal** auto-remplis **(éditables, pas lecture seule — voir spike)** et le **code postal affiché après sélection** pour confirmer, afin que l'autocomplétion fonctionne et que l'adresse soit fiable. | 5 | **Should** | ✅ **LIVRÉ (2026-06-23, EPIC 15)** — champ unique « Adresse (n° et rue) » (= autocomplete address-line1) sur les 5 écrans (caisse/location/installation/profil/mesurer) ; contrôle civicNumber séparé supprimé ; ville/province/CP auto-remplis ÉDITABLES ; voie serveur B1 (split à la présentation via splitAddressLine, AddressDto serveur inchangé, ZÉRO migration EF) ; province en `<select>` 2-lettres partout (L-011) ; tokens autocomplete WCAG 1.3.5 ; ids préfixés (L-013). Revue indépendante APPROVE WITH NITS. |
| US-15.3 | En tant que **client**, je veux que **modifier le n° civique mette à jour la rue/le code postal** (cohérence) afin d'éviter une adresse incohérente. | 1 | **Should** | ✅ **LIVRÉ (2026-06-23, EPIC 15)** — dissous par construction : le n° civique faisant désormais partie du champ adresse unifié, il ne peut plus se désynchroniser de la rue. |
