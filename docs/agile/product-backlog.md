...
| US-7.1 | En tant que **client**, je veux **payer ma commande/location/installation par Interac e-Transfer** (demande par courriel/téléphone/ID) afin de payer sans carte de crédit. | 13 | **Could** | 🔵 **Livré (commit local 2026-06-23, branche `feat/epic-7-paiement-etransfer`)** — tunnel checkout e-Transfer + admin « marquer payé » + i18n fr/en + e2e dual-thème. Revue `code-reviewer` APPROVE WITH NITS. |
