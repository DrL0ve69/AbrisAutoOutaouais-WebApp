...
| US-7.1 | En tant que **client**, je veux **payer ma commande/location/installation par Interac e-Transfer** (demande par courriel/téléphone/ID) afin de payer sans carte de crédit. | 13 | **Could** | 🔵 **En cours (branche `feat/epic-7-paiement-etransfer`)** — **Tranche Commandes (7.1, commit local 2026-06-23)** : tunnel checkout e-Transfer + admin « marquer payé » + i18n fr/en + e2e dual-thème ; revue `code-reviewer` APPROVE WITH NITS. **Tranche Locations (7.2, commit local 2026-06-23)** : `RentalStatus.PendingPayment → Active`, VO `PaymentInfo`/`ConfirmRentalPaymentCommand` (AdminOnly), migration `AddRentalContractPaymentInfo`, tunnel `/location` panneau e-Transfer (L-053/L-006), admin rentals « Marquer payé » (L-024), `account/rentals` statut, 16 ids i18n EN manuels (L-042), e2e `location-etransfer`+`admin-rentals-payment` (axe dual-thème, L-051). **Reste : Tranche Installations (7.3) + stubs/doc webhook (7.4).** Revue indépendante à la frontière d'épic. |
