...
- **Prochaine sous-tâche :** **Épic E entièrement livré et revu — frontière d'épic en cours : PR → CI → merge `master`, puis Épic F.** Après le merge, la prochaine sous-tâche est **Épic F (wrap-up docs/process)** sur la branche `docs/program-wrapup` (voir « Suite du programme »).
...
- **⚑ Frontière d'épic E — RESTE À FAIRE avant merge** : (1) **round-trip live L-001** manuel (à l'œil, par l'utilisateur) : reduced-motion ON/OFF réel, **les deux thèmes**, glass-on-scroll, overlay navigation, cursor-ring pointeur-fin, viewer 3D + repli ; (2) **relevé Lighthouse manuel** ; puis **commit → PR → CI verte → merge `master`** ([[program-git-flow]]). Revue indépendante `code-reviewer` E1→E5 : ✅ faite (APPROVE WITH NITS).
...
- **Dernière mise à jour :** 2026-06-13 (E5 perf gates + remédiation contraste fait — **clôture Épic E** ; 2 régressions contraste corrigées au niveau jeton, `e2e/motion-a11y.spec.ts`, reschedule pleine page ; revue indépendante APPROVE WITH NITS ; curseur → frontière d'épic E : round-trip live L-001 + Lighthouse manuels, puis PR → CI → merge `master`, ensuite Épic F)
