...
- **Prochaine sous-tâche :** **E5 — Perf gates + remédiation contraste (clôture Épic E)**. Deux volets :
  - **(a) MUST-FIX a11y — régression contraste du redesign Épic E (barre dure WCAG 1.4.3 AA).** Découverte en E4 : l'e2e Playwright (couleurs réellement composées) révèle des violations `color-contrast` sur `.navbar--scrolled` (verre `rgba(15,25,35,0.82)`) + sections **admin / rentals / mesurer**, introduites par E1 (tokens v2) / E3 (navbar glass). **Pourquoi non détecté plus tôt** : `npm test` vitest **désactive `color-contrast`** (`src/testing/axe-helper.ts:14`, styles globaux non chargés en unitaire) → les « zéro axe » d'E1/E2/E3 étaient **vacueux pour le contraste** (cf. **leçon L-016** capturée). Corriger les couleurs fautives dans les **deux thèmes**, re-vérifier en e2e + round-trip live. Attention **L-017** : tuer tout `ng serve` zombie sur 4200/4300 avant de juger l'e2e (sinon code périmé).
  - **(b) E5 nominal** : figer le bundle initial (toutes libs lourdes lazy — déjà le cas : gsap E2, three E4) ; **nouveau `e2e/motion-a11y.spec.ts`** (émulation `prefers-reduced-motion` réelle : pas de pin hero, scène figée, pas de cursor-ring, pas d'auto-rotate 3D ; **axe DEUX thèmes — couvre le volet (a)**) ; gate Lighthouse manuel LCP ≤2,5 s / CLS ≤0,1 / INP ≤200 ms ; réparer tout sélecteur e2e cassé par le redesign dans les mêmes commits.
- **⚑ Frontière d'épic E (après E5)** : revue indépendante `code-reviewer` couvrant **E1→E5** (l'implémenteur ne valide jamais son diff) + round-trip live L-001 complet (reduced-motion ON/OFF réel, **les deux thèmes**, glass-on-scroll, overlay navigation, cursor-ring pointeur-fin, viewer 3D + repli) → puis **revue → commit → PR → CI → merge `master`** ([[program-git-flow]]).
...
- **Dernière mise à jour :** 2026-06-13 (E4 viewer 3D fait — commit local `02c69d2` ; curseur → E5 perf gates + **must-fix contraste** Épic E ; leçons L-016/L-017 capturées)
