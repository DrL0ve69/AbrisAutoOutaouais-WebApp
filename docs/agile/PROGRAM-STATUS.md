...
- **Prochaine sous-tâche :** **E3 — Micro-interactions** : directives `shared/directives/{reveal-on-scroll (IntersectionObserver), magnetic-hover (pointer:fine), count-up}` ; composant `shared/components/loading-overlay` (spinner SVG arche d'abri, `aria-live polite`, statique sous reduced-motion) câblé aux événements router ; composant `shared/components/cursor-ring` (`pointer:fine` only, overlay `pointer-events:none`, **jamais `cursor:none`**, off sous reduced-motion, `aria-hidden`) ; navbar glass-on-scroll. Consomme les tokens de mouvement E1. **Spec vitest par directive incluant la branche reduced-motion** (assertion mode-indépendante + hook ; l'émulation media réelle est laissée à Playwright/E5 — cf. note ci-dessous). **Gate = axe dual-theme à zéro.**
...
- **Dernière mise à jour :** 2026-06-13 (E2 hero scroll story fait — commit local `651afa0` ; curseur → E3 micro-interactions)
