# Journal de livraison — AbrisTempo Local (archive chronologique)

> **Ce que c'est.** L'historique chronologique des clôtures d'épopées et de sprints, déplacé hors de
> `board.md` (qui ne garde que l'instantané courant + le registre des bogues/dette). Pour l'état
> narratif vivant du programme, voir **`PROGRAM-STATUS.md`** (source de vérité unique). Entrées du
> plus ancien (Sprint 4, 2026-06-11) au plus récent (clôture Phase 2, 2026-06-23).
...
| EPIC 7 | Paiements (Interac e-Transfer + cartes) | 1 | — | 21+ | Could | ✅ **MERGÉ (2026-06-23, PR #59, SHA `45b8157`)** — 7.0→7.4 livrés (e-Transfer 3 flux + stubs keyless + doc webhook), revue de fin d'épic APPROVE WITH NITS. **EPIC 7 = dernière épopée Phase 2 → Phase 2 TERMINÉE.** |
...
| EPIC 15 | Champ d'adresse unifié (spike→reco) | (1)·5.2 | — | 8 | Should | ✅ **LIVRÉ + MERGÉ (2026-06-23, PR #60 → `master` `946af7a`, CI verte, branche supprimée)** — US-15.1 spike (LIVRÉ 2026-06-18) + **US-15.2/15.3 livrés** : champ unique « n°+rue » sur 5 écrans, voie serveur B1 (`splitAddressLine`, AddressDto intact, zéro migration), province `<select>` 2-lettres (L-011), ids préfixés (L-013), tokens autocomplete WCAG 1.3.5. Revue indép. APPROVE WITH NITS (demande traitée). Leçon L-055. **Round-trip live LocalDB ✅** (profil : adresse QC scindée `111`+`rue de l'Atmosphère` persistée exacte, Ontario→200 sans 422 L-004, civique vide→non persisté ; commande livrée 201 → colonnes `ShippingAddress_CivicNumber=111` **et** `_Street` distinctes vérifiées en SQL = intégrité B1 bout-en-bout). |
