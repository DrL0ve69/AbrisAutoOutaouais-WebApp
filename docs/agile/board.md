...
| Bug-08(F2-D) | `/mesurer` : dessin sur carte non fonctionnel — `map.pm` (geoman) ne s'attache pas | 4.1.2 / fonctionnel | **À corriger** : geoman patche une instance Leaflet ≠ de celle créée par l'import dynamique (interop CJS/ESM, axe `map.pm`, cousin du « `L.map` is not a function ») → binder geoman sur la même instance. Préexistant Épic D, masqué par un smoke test conteneur-seul ; exposé par la garde positive `.leaflet-pm-toolbar` (épinglée `test.fixme`, L-019) | 🔴 Ouvert (suivi) |
