...
## L-055 · A native `<select>` added to a screen that also hosts an APG combobox makes every unscoped `getByRole('option')` ambiguous — scope all option queries to the specific listbox

- **Symptom.** EPIC 15 (unified address) converted the `province` form control from a text `<input>` to a native `<select>` over 13 Canadian province codes on all address screens (checkout, location, installation, profile, map-voie). Each `<select>` renders ~13 `role="option"` DOM nodes. Those screens also host `app-address-autocomplete`, an APG combobox whose suggestion listbox also exposes `role="option"` nodes. After the change, unscoped `getByRole('option')` / `findAllByRole('option')` queries in `map-voie.spec.ts` and `e2e/address-autocomplete.spec.ts` started matching the `<select>`'s option nodes instead of (or in addition to) the combobox suggestions. The combobox suggestion was never selected → `lat === null` in vitest, and a mis-count of suggestion nodes in e2e. Diagnosed via a temporary debug log (`options=13, suggestCalls=0`); fixed by scoping option queries to the combobox's listbox (`within(listbox)` / `#…-listbox [role="option"]`). No compile error, no build warning — only `npm test` failing exposed it.
- **Rule.** When you add a native `<select>` (or any `role="listbox"`-bearing control — multiselect, `<datalist>`) to a screen that already hosts an APG combobox/listbox, treat it as a **breaking change to every option-role query** on that screen: a `<select>`'s `<option>` elements expose the implicit ARIA role `option` and join the same page-global role namespace as the combobox's suggestion nodes. Two guards: (1) After adding any `<select>` to a screen, **grep that screen's specs** (vitest + e2e) for bare `getByRole('option')`, `findAllByRole('option')`, and `getByRole('listbox')` — re-scope each one to the specific listbox container (`within(comboboxListbox)` or a CSS-scoped selector `#<id>-listbox [role="option"]`). (2) In any spec that asserts « suggestion X was offered » or « N suggestions appeared », make the count and the selection target the **combobox's listbox node explicitly**, never the whole document. A wrong count (e.g. 13 instead of the expected 3) is the diagnostic tell for this ambiguity. This is a direct cousin of [[L-048]] (a second owner of the same text/role makes a bare query match multiple nodes) and [[L-010]] (a new global role node widens the match set of unrelated role locators), on the **native-`<select>`-option** axis.
- **Refs.**
  `src/AbrisAutoOutaouais-WebApp.Client/src/app/features/location/location.spec.ts` + `features/mesurer/steps/dimension-step/map-voie/map-voie.spec.ts` (option queries scoped via `within(listbox)`),
  `e2e/address-autocomplete.spec.ts` (counts `#loc-address-line1-listbox [role="option"]`),
  province `<select>` added to checkout/location/installation/profile/map-voie HTML templates,
  branch `feat/epic-15-adresse-unifiee`. Related: [[L-048]] (second owner of same role/text), [[L-010]] (global role node widens unrelated locators).
