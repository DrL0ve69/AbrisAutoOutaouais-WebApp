...
/// <summary>
/// Adaptateur <see cref="IPaymentService"/> pour VoPay (API open-banking canadienne, Interac
/// e-Transfer). STUB KEYLESS, NON IMPLÉMENTÉ ET JAMAIS LE DÉFAUT : il existe uniquement pour
...
/// <para>
/// L'intégration RÉELLE de VoPay exige une clé API, un contrat marchand réglementé (KYC/AML) et des
/// frais par transaction — donc un compte de facturation → <b>traité comme PAYANT</b> par
/// <c>.claude/rules/budget-free-tier.md</c>. Le vrai appel API REST n'est livré que sous l'accord
/// explicite du propriétaire. Tant que la clé est vide, la garde fail-fast de
...
/// </para>
/// </summary>
internal sealed class VoPayPaymentService(IOptions<PaymentsOptions> options) : IPaymentService
{
    // La clé n'est lue que pour matérialiser le point d'extension ; le stub n'effectue aucun appel.
    private readonly VoPayPaymentOptions _vopay = options.Value.VoPay;
    ...
    {
        var instructions =
            "STUB VoPay — adaptateur de paiement NON IMPLÉMENTÉ et NON ACTIVÉ. Ce fournisseur " +
            "n'encaisse aucun paiement : il documente uniquement le point d'extension du port de " +
            "paiement. L'intégration réelle de VoPay (API Interac e-Transfer) exige une clé API, un " +
            "contrat marchand payant et l'accord explicite du propriétaire (règle budget). Le " +
            "fournisseur par défaut reste le virement Interac manuel (« manual »).";
